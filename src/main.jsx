...
const saved = JSON.parse(localStorage.getItem('palpiteiro_session') || 'null');
...
function App() {
  ...
  useEffect(() => {
    const state = (next) => { setRoom(next); setView(next.phase); setSent(false); setGuess(''); setSecondsLeft(next.seconds); };
    const onError = (message) => setError(message);
    socket.on('room_state', state); socket.on('game_error', onError); socket.on('answer_received', () => setSent(true));
    ...
    return () => { socket.off('room_state', state); socket.off('game_error', onError); };
  }, []);
  ...
  if (view === 'connecting') return <Shell><div className="loading">Reconectando à sua mesa...</div></Shell>;
  if (view === 'home') return <Home name={name} setName={setName} code={code} setCode={setCode} create={create} join={join} error={error} />;
  if (view === 'lobby') return <Lobby room={room} me={me} settings={settings} setSettings={setSettings} toggleCategory={toggleCategory} copyCode={copyCode} start={() => socket.emit('start_game', settings)} error={error} />;
